                ...
                <div className="flex gap-1 mb-4 bg-gray-100 p-1 rounded-lg w-fit">
                    <button
                        ...
                        className={cn(
                            ...
                            activeTab === "carga" ? "bg-white text-primary shadow-sm" : "text-gray-500 hover:text-gray-700"
                        )}
                    >
                    ...
                    <button
                        ...
                        className={cn(
                            "px-4 py-2 rounded-md text-sm font-bold transition-all flex items-center gap-1.5",
                            activeTab === "historial" ? "bg-white text-primary shadow-sm" : "text-gray-500 hover:text-gray-700"
                        )}
                    >
                ...
                <div className="h-[calc(100vh-220px)] flex flex-col">
                    ...
                    <div className="grid grid-cols-3 gap-3 mb-4 shrink-0">
                        <Card className="border-gray-200 bg-white shadow-sm">
                            <CardContent className="p-3">
                                <p className="text-[10px] text-gray-400 uppercase font-bold">Total Cargas</p>
                                <p className="text-lg font-black text-gray-800">{filteredHistory.length}</p>
                            </CardContent>
                        </Card>
                        <Card className="border-gray-200 bg-white shadow-sm">
                            <CardContent className="p-3">
                                <p className="text-[10px] text-gray-400 uppercase font-bold">Total Unidades</p>
                                <p className="text-lg font-black text-gray-800">{filteredHistory.reduce((sum, h) => sum + (h.totalItems || 0), 0).toLocaleString()}</p>
                            </CardContent>
                        </Card>
                        <Card className="border-gray-200 bg-white shadow-sm">
                            <CardContent className="p-3">
                                <p className="text-[10px] text-gray-400 uppercase font-bold">Costo Total</p>
                                <p className="text-lg font-black text-green-600">${filteredHistory.reduce((sum, h) => sum + (h.totalCost || 0), 0).toLocaleString()}</p>
                            </CardContent>
                        </Card>
                    </div>
                    ...
                    <div className="flex items-center gap-3 mb-3 shrink-0">
                        ...
                        <span className="text-[11px] text-gray-400">{filteredHistory.length} cargas</span>
                    </div>
                                    ...
                                    <tbody className="divide-y divide-gray-100">
                                        {filteredHistory.map(load => (
                                            <tr key={load.id} className="hover:bg-gray-50">
                                                <td className="p-3 font-black text-gray-800 text-xs">#{String(load.id).padStart(3, '0')}</td>
                                                <td className="p-3 text-[11px] text-gray-500">
                                                    {(() => { const d = (load as any).created_at || load.createdAt; return d ? new Date(d).toLocaleDateString('es-CO', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : '-' })()}
                                                </td>
                                                <td className="p-3 text-xs font-semibold text-gray-700">{load.vendors?.name || '-'}</td>
                                                <td className="p-3 text-[11px] text-gray-500">{load.referenceCode || '-'}</td>
                                                <td className="p-3 text-center text-xs">{load.totalItems} uds</td>
                                                <td className="p-3 text-right text-xs font-bold text-green-600">${load.totalCost?.toLocaleString()}</td>
                                                <td className="p-3 text-center">
                                                    <Badge variant={load.status === 'completed' ? 'default' : load.status === 'cancelled' ? 'destructive' : 'secondary'} className="text-[9px] h-4">
                                                        {load.status === 'completed' ? 'Completada' : load.status === 'cancelled' ? 'Anulada' : load.status}
                                                    </Badge>
                                                </td>
                                                <td className="p-3 text-center">
                                                    <div className="flex items-center justify-center gap-1">
                                                        <button onClick={() => { setDetailLoad(load); setShowDetail(true) }} className="text-gray-400 hover:text-primary p-1">
                                                            <Eye className="w-4 h-4" />
                                                        </button>
                                                        <button onClick={() => handleDeleteLoad(load.id)} className="text-gray-400 hover:text-red-500 p-1">
                            ...
                            <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-4 text-sm">
                                    ...
                                    <div>
                                        ...
                                        <p>{(() => { const d = (detailLoad as any).created_at || detailLoad.createdAt; return d ? new Date(d).toLocaleString('es-CO') : '-' })()}</p>
                                    </div>
                                    <div>
                                        ...
                                        <table className="w-full text-sm border border-gray-200 rounded-lg overflow-hidden">
                                            ...
                                            <tbody className="divide-y divide-gray-100">
                                                {detailLoad.items.map((item: any) => {
                                                    return (
                                                        <tr key={item.id}>
                                                            <td className="p-3">
                                                                <p className="font-bold text-xs">{item.products?.productName || `Prod #${item.productId}`}</p>
                                            ...
                                            <tfoot>
                                                <tr className="bg-gray-100 font-black text-[11px]">
                                                    <td className="p-3 text-left" colSpan={3}>TOTALES</td>
                                                    <td className="p-3 text-right text-green-700">${detailLoad.items.reduce((sum: number, item: any) => sum + (item.totalCost || 0), 0).toLocaleString()}</td>
                                                </tr>
                                            </tfoot>
                                        </table>
                                    </div>
